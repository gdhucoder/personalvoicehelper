        ...
        while True:
            ...
            if result >= 0:
                ...
                # 关于提醒
                    ...
                    reminder_manager.add(at_time, f"提醒：{when.hour}点，{when.minute}分，{txt}")

                    loop.call_soon_threadsafe(scheduler.enqueue, SpeakTextTask(msg))
                elif intent == "remove_reminder":
                    idx = params['idx']
                    ok = reminder_manager.remove(idx)
                    msg = ok and f"已删除第{idx}条提醒" or "未找到提醒"
                    loop.call_soon_threadsafe(scheduler.enqueue, SpeakTextTask(msg))
                elif intent == "list_reminders":
                    lst = reminder_manager.list()
                    if not lst:
                        loop.call_soon_threadsafe(scheduler.enqueue, SpeakTextTask("当前没有待提醒事项"))
                    for i, rm in enumerate(lst, 1):
                        reminder_txt = f"第{i}条， {rm.at_time}, {rm.message}"
                        loop.call_soon_threadsafe(scheduler.enqueue, SpeakTextTask(reminder_txt))
                elif intent == "weather":
                    # 将 WeatherTask 加入调度器
